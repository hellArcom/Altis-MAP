...
class _MapHomeScreenState extends State<MapHomeScreen> {
  ...
  // Tuiles détectées automatiquement
  String? _vectorMapPath;
  String? _terrainMapPath;

  // Caméra courante pour la détection dynamique de tuiles
  ...
  /// Scanne les fichiers disponibles et sélectionne automatiquement
  /// le meilleur vecteur et le meilleur terrain.
  Future<void> _loadOfflineMap() async {
    ...
    // Priorité : fichier le plus gros qui n'est pas terrain = vecteur
    final vectorFiles = files
        .where((f) => !f.path.contains('terrain') && !f.path.contains('dem'))
        .toList()
      ..sort((a, b) => b.lengthSync().compareTo(a.lengthSync()));

    final terrainFiles = files
        .where((f) => f.path.contains('terrain') || f.path.contains('dem'))
        .toList()
      ..sort((a, b) => b.lengthSync().compareTo(a.lengthSync()));

    _vectorMapPath = vectorFiles.isNotEmpty ? vectorFiles.first.path : null;
    _terrainMapPath = terrainFiles.isNotEmpty ? terrainFiles.first.path : null;
  ...
  /// Reconstruit le style JSON en fonction des options actives.
  Future<void> _updateMapStyle() async {
    if (_vectorMapPath == null || _vectorMapPath!.isEmpty) {
      // Pas de fichier vecteur : on remet à null pour afficher le placeholder
    ...
    final Map<String, dynamic> sources = {
      'offline-source': {
        'type': 'vector',
        'url': 'mbtiles://$_vectorMapPath',
        // On ne spécifie PAS maxzoom ici : MapLibre lira la valeur réelle
        // depuis les métadonnées du fichier .mbtiles et activera l'overzoom
        // automatiquement (les tuiles zoom 14 seront réutilisées jusqu'au
        // zoom 22, évitant ainsi les rectangles beiges).
        'minzoom': 0,
      },
    };
    ...
    final List<Map<String, dynamic>> layers = [
      {
        // Le fond utilise la même couleur que la terre pour que les zones
        // hors-couverture soient invisibles (pas de rectangles beiges).
        'id': 'background',
        'type': 'background',
        'paint': {
    ...
    if (_isSatelliteView) {
      ...
      // Couches vectorielles de base (format Shortbread)
      // maxzoom: 22 sur chaque couche = overzoom activé (tuiles z14 réutilisées
      // jusqu'au zoom 22, ce qui supprime les rectangles vides à fort zoom).
      layers.addAll([
        {
          'id': 'land-sb',
          'type': 'fill',
    ...
    if (_showHillshade && _terrainMapPath != null && _terrainMapPath!.isNotEmpty) {
      final terrainUrl = _terrainMapPath!.startsWith('/')
          ? 'mbtiles://$_terrainMapPath'
          : 'mbtiles:///$_terrainMapPath';
    ...
    // Eau (compatibilité Shortbread + OpenMapTiles)
    layers.addAll([
      {
        'id': 'water-sb',
        'type': 'fill',
    ...
    // Hillshade (après l'eau, avant les routes)
    if (_showHillshade && _terrainMapPath != null && _terrainMapPath!.isNotEmpty) {
      layers.add({
        'id': 'hillshade',
        'type': 'hillshade',
    ...
    // Terrain 3D uniquement si hillshade activé et fichier DEM présent
    if (_showHillshade && _terrainMapPath != null && _terrainMapPath!.isNotEmpty) {
    ...
    if (mounted) {
      ...
      // La ValueKey sur MapLibreMap force la reconstruction automatique
      // quand _mapStyleString change — pas besoin d'appel supplémentaire.
    }
  }

  // ── Chargement dynamique des tuiles selon la vue ──────────────────────────

  /// Appelé à chaque mouvement de caméra : détecte si une autre tuile
  /// couvre mieux la zone visible et change automatiquement.
  Future<void> _onCameraIdle() async {
    ...
    // Cherche le fichier qui couvre le mieux la zone visible
    final mapsDir = await _getMapsDirectory();
    final allFiles = _findMbtilesRecursive(mapsDir);

    final vectorFiles = allFiles
        .where((f) => !f.path.contains('terrain') && !f.path.contains('dem'))
        .toList();

    if (vectorFiles.isEmpty) return;

    // Pour chaque fichier, on vérifie s'il existe un .poly ou on se base
    // sur le nom du répertoire. Heuristique : on sélectionne le plus petit
    // fichier qui couvre la zone (supposé plus détaillé localement).
    // Sans métadonnées de bounding box, on utilise le fichier déjà sélectionné.
    // Cette logique peut être enrichie avec la lecture de métadonnées MBTiles.
    final center = LatLng(
      (bounds.southwest.latitude + bounds.northeast.latitude) / 2,
      (bounds.southwest.longitude + bounds.northeast.longitude) / 2,
    );

    // Tente de trouver un fichier dont le nom correspond à la région visible
    // (ex: "ile-de-france", "bretagne", etc.)
    File? bestFile;
    for (final file in vectorFiles) {
      ...
      // Si le fichier est déjà sélectionné, on passe
      if (file.path == _vectorMapPath) {
        bestFile = file;
        break;
      }
    }

    // Si aucun meilleur fichier trouvé, on garde l'actuel
    if (bestFile != null && bestFile.path != _vectorMapPath) {
      setState(() => _vectorMapPath = bestFile!.path);
      await _updateMapStyle();
    }
  }
          ...
          child: Column(
            ...
            children: [
              ...
              SwitchListTile(
                ...
                onChanged: _terrainMapPath == null
                    ? null
                    : (val) {
              ...
              if (_showHillshade && _terrainMapPath != null) ...[
                Padding(
      ...
      body: Stack(
        children: [
          ...
          if (_mapStyleString != null)
            Positioned.fill(
              child: MapLibreMap(
                key: ValueKey('map_$_vectorMapPath'),
